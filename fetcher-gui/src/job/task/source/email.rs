...
use fetcher_config::jobs::source::email::{Auth, Email, Filters, ViewMode};
...
pub fn show(email: &mut Email, task_id: impl Hash, ui: &mut Ui) {
	...
	ui.label("Filters");
	ui.group(|ui| show_filters(filters, ui));
...
fn show_filters(filters: &mut Filters, ui: &mut Ui) {
	{
		// sender
...

	{
		...
		ui.add_enabled_ui(exclude_subjects_enabled, |ui| {
			...
			if exclude_subjects_enabled {
				ui.horizontal(|ui| {
					let exclude_subjects = filters.exclude_subjects.get_or_insert_with(Vec::new);
